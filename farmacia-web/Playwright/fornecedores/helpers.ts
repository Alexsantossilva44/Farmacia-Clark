...
export async function irParaFornecedores(page: Page) {
  await page.goto('/cadastros?aba=fornecedores')
  await expect(
    page.getByRole('heading', { name: /Novo fornecedor|Editar fornecedor/i }),
  ).toBeVisible({ timeout: 15_000 })
}

export function botaoCadastrar(page: Page) {
  return page.getByRole('button', { name: 'Cadastrar fornecedor' })
}
...
export function gerarRazaoSocial(seed: number): string {
  return `Distribuidora Teste ${seed}`
}

// ─── API helpers ──────────────────────────────────────────────────────────────

export async function cadastrarFornecedorApi(
  request: APIRequestContext,
  token: string,
  payload: { razaoSocial: string; cnpj: string; nomeFantasia?: string },
) {
  const res = await request.post(`${apiBase}/api/v1/fornecedores`, {
