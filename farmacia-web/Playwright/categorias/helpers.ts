...
export async function irParaCategorias(page: Page) {
  await page.goto('/cadastros?aba=categorias')
  await expect(
    page.getByRole('heading', { name: /Nova Categoria|Editar Categoria/i }),
  ).toBeVisible({ timeout: 15_000 })
}

export function botaoCadastrar(page: Page) {
  return page.getByRole('button', { name: 'Cadastrar categoria' })
}

// ─── Geradores ────────────────────────────────────────────────────────────────

export function gerarNomeCategoria(seed: number): string {
  return `Categoria Teste ${seed}`
}

export function gerarDescricao(seed: number): string {
  return `Descrição de teste para categoria ${seed}`
}

// ─── API helpers ──────────────────────────────────────────────────────────────

export async function cadastrarCategoriaApi(
  ...
  return res.json() as Promise<{ id: string; nome: string; descricao: string }>
}
